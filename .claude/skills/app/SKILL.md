...
1. Spusti Flask server **na pozadí** (run_in_background = true), aby neblokoval ďalšiu prácu.
   Spusti **presne** príkaz `python app.py` (Bash) — **bez** `cd` a bez iných prefixov, aby sedel
   s vopred povoleným pravidlom a nepýtal povolenie.
...
3. Stručne povedz používateľovi:
   ...
   - ako ho **zastaviť**: priamo na stránke je tlačidlo **„🛑 Vypnúť server"** (najpohodlnejšie),
     alebo stačí povedať „zastav appku" (zastavíš proces na pozadí), prípadne `Ctrl+C` v termináli.
